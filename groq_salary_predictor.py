...
def predict_salary_with_groq(form_data: dict, api_key: Optional[str] = None) -> dict:
    ...
    # Try to create client with error handling for version issues
    try:
        client = Groq(api_key=api_key)
    except TypeError as e:
        if "proxies" in str(e):
            # Old version of Groq library - provide helpful error
            raise ValueError(
                "Outdated Groq library detected. "
                "Please upgrade: pip install --upgrade groq>=0.11.0 "
                "Or reboot your Streamlit Cloud app to clear cache."
            )
        raise
...
def compare_predictions(xgboost_result: dict, groq_result: dict) -> dict:
    ...
    """
    # Handle both 'predicted' and 'predicted_salary' keys
    xgb_pred = xgboost_result.get('predicted') or xgboost_result.get('predicted_salary', 100000)
    groq_pred = groq_result.get('predicted_salary') or groq_result.get('predicted', 100000)
